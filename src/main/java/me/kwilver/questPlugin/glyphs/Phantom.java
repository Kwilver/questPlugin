...
public class Phantom extends Glyph implements Listener {
    ...
    protected boolean useGlyph() {
        ...
        attackListener = new Listener() {
            ...
            public void onPlayerSwing(PlayerInteractEvent event) {
                ...
                RayTraceResult r = eye.getWorld().rayTraceEntities(
                        eye, dir, 5,
                        e -> e instanceof LivingEntity && !e.equals(fakePlayer.getBukkitEntity())
                );

                if (r != null && r.getHitEntity() instanceof LivingEntity) {
                    LivingEntity target = (LivingEntity) r.getHitEntity();

                    double dmg = user.getAttribute(Attribute.ATTACK_DAMAGE)
                            .getValue();
                    target.damage(dmg, fakePlayer.getBukkitEntity());

                    fakePlayer.getBukkitEntity()
                            .swingHand(EquipmentSlot.HAND);
                }
            }
        };
        QuestPlugin.getInstance().getServer()
                .getPluginManager()
                .registerEvents(attackListener, QuestPlugin.getInstance());
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (last != null) {
                    Vector movement = user.getLocation().toVector()
    ...
    public void onPlayerDeath(PlayerDeathEvent e) {
        if(e.getPlayer() == fakePlayer.getBukkitEntity()) {
            e.getDrops().clear();
        }
    }
}
